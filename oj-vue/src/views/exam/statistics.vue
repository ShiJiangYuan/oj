    ...
    <el-input v-model="query" clearable placeholder="请输入考试名称" class="input-with-select" style="width:300px;margin-right:20px">
     ...
     <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="操作" width="200px">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.row.id)">成绩分析 <i class="el-icon-pie-chart"></i></el-button>
...
export default {
  ...
  methods: {
    ...
    getExams () {
      ...
      this.$axios.get('exam/getFinishedExams', {
        params: {
    ...
    handleEdit (id) {
      const p = this.$router.resolve({ path: `/analyse/${id}` })
