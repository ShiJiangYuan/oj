<template>
  <div>
    ...
    <el-table :data="tableData" stripe style="width: 100%;margin-top:10px" v-loading="loading" border>
      ...
      <el-table-column prop="sure"  label="参考答案" width="200px">
      </el-table-column>
...
@current-change="handleCurrentChange">
    ...
    <el-dialog :close-on-click-modal="false" :title="save" :visible.sync="dialogAddVisible" width="500px">
      <el-form ref="form" status-icon :model="form" :rules="rules">
        ...
        <el-form-item label-position="right" label="参考答案" label-width="80px" prop="sure">
          <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}"  v-model="form.sure"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      // url
      urlImport: this.$axios.defaults.baseURL + '/programme/import'
    }
  },
  methods: {
    // 导入数据提示
    lend () {
      this.$message('导入格式（带标题）：题目|参考答案|题目来源(可不填)，只允许上传xlsx文件')
    },
    // 导入数据失败
    ...
    async deleteClick (id) {
      try {
        ...
        const { code, msg } = await this.$axios.delete(`programme/deleteById/${id}`)
        if (code === '200') {
    ...
    async addTitle () {
      try {
        ...
        const res = await this.$axios[apiType]('programme/add', this.form)
        if (res.code === '200') {
