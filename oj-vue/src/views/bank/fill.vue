...
export default {
  ...
  data () {
    return {
      ...
      // url
      urlImport: this.$axios.defaults.baseURL + '/fill/import'
    }
  },
  methods: {
    // 导入数据提示
    lend () {
      this.$message('导入格式（带标题）：题目|正确选项|题目来源(可不填)，只允许上传xlsx文件')
    },
    // 导入数据失败
    ...
    async deleteClick (id) {
      try {
        ...
        const { code, msg } = await this.$axios.delete(`fill/deleteById/${id}`)
        if (code === '200') {
    ...
    async addTitle () {
      try {
        ...
        const res = await this.$axios[apiType]('fill/add', this.form)
        if (res.code === '200') {
